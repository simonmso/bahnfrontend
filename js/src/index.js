...
const formatStopsFromTimetable = (xmlDoc) => (
  // this spread lets us use the map function
  [...xmlDoc.firstChild.childNodes.values()]
    .map((n) => nodeToStop(n))
);
...
const getPlanForTime = (evaNo, dateArg) => {
  ...
  return request(`/plan/${evaNo}/${date}/${hour}`)
    .then((resp) => formatStopsFromTimetable(resp));
};

const main = async () => {
  const evaNo = 8098160;
  const plan = await getPlanForTime(evaNo);
