...
const names = ["Dresden-Neustadt", "Ruhland", "Ortrand", "Lampertswalde", "Großenhain Cottb Bf", "Priestewitz", "Weinböhla Hp", "Coswig(b Dresden)"];
...
const stops = names.map((n, i) => ({
  ...
  departureTime: withs[i][1] ? now.add({ hours: add[i] }).with({ minute: withs[i][1] }) : undefined,
}));
